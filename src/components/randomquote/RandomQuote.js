...
function Quotes() {
  ...
  function handleClickNewQuote() {
    const colorsBackGround = ['bg-red-300', 'bg-green-300', 'bg-blue-300', 'purple-400', 'bg-pink-300', 'bg-yellow-200']
    const colorsText = ['text-red-300', 'text-green-300', 'text-blue-300', 'text-purple-400', 'text-pink-300', 'text-yellow-200']
    const ColorIndex = Math.floor(Math.random() * colorsBackGround.length)

    setcolorBackGround(colorsBackGround[ColorIndex])
    ...
    <div className={colorBackGround}>
      <div className="bg-white w-5/12 p-16 rounded-3xl mx-auto shadow-2xl mt-10">
        ...
        <div className="flex justify-between mt-5">
          ...
          <div className={colorBackGround}>
           <button onClick={handleClickNewQuote} className="p-2 border-2">New Quote</button>
          </div>        
        </div> 
    </div>
    </div>
